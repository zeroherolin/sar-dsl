#!/usr/bin/env python3
"""Range-Doppler on the CPU backend, end to end: simulate a synthetic
point-target scene, compile the kernel, focus it and save a PNG.

Usage:
    python run_point_target_cpu.py [--n 512] [--output rda_cpu.png]
"""
...
def main() -> None:
    ...
    parser.add_argument("--output", default="rda_cpu.png")
    ...
    print("[2/3] Compiling and running the RDA kernel (cpu backend) ...")
    ...
    save_db_image(
        image, str(output),
        f"SAR-DSL Range-Doppler (cpu), {n}x{n} synthetic point targets")
    print("done.")
