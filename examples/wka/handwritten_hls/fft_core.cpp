...
static void load_transform_block(bus_t *__restrict in,
                                 data_t buffer[WKA_FFT_PAR_ROWS][N],
                                 int base_row, bool load_shift,
                                 bool apply_window) {
#pragma HLS INLINE off
#pragma HLS ARRAY_PARTITION variable = WKA_AZIMUTH_WINDOW_ROM cyclic factor =  \
    BUS_LANES dim = 1
#pragma HLS BIND_STORAGE variable = WKA_AZIMUTH_WINDOW_ROM type =              \
    rom_2p impl = bram
...
#pragma HLS UNROLL
        ...
        if (apply_window) {
          ...
          value =
              data_t(value.real() * coefficient, value.imag() * coefficient);
        }
...
static void store_transform_block(data_t buffer[WKA_FFT_PAR_ROWS][N],
                                  bus_t *__restrict out, int base_row,
                                  bool store_shift, bool inverse) {
#pragma HLS INLINE off
...
static void run_row_transform_impl(bus_t *__restrict in, bus_t *__restrict out,
                                   bool inverse, bool load_shift,
                                   bool store_shift, bool apply_window,
                                   bool fused_stolt) {
#pragma HLS INLINE off
...
#pragma HLS INLINE off
  ...
  run_row_transform_impl(in, out, inverse, load_shift, store_shift,
                         apply_window, fused_stolt);
}
