...
#pragma HLS ALLOCATION operation instances = fmul limit = WKA_STOLT_READ_FMUL_LIMIT
#pragma HLS BIND_STORAGE variable = WKA_TWIDDLE_COS_ROM type = rom_2p impl = bram
#pragma HLS BIND_STORAGE variable = WKA_TWIDDLE_SIN_ROM type = rom_2p impl = bram
...
#pragma HLS BIND_STORAGE variable = packed_row type = ram_t2p impl = WKA_STOLT_PACKED_STORAGE_IMPL
...
PROCESS_GROUPS:
    for (int group_index = 0; group_index < N / WKA_STOLT_OUT_LANES; group_index++) {
#pragma HLS PIPELINE II = WKA_STOLT_READ_WRITE_II
...
#pragma HLS UNROLL
            ...
            data_t value = unpack_data(
                static_cast<uint64_t>(packed.range((packed_lane + 1) * WKA_COMPLEX_SAMPLE_BITS - 1,
                                                   packed_lane * WKA_COMPLEX_SAMPLE_BITS)));

            real_t fr = fr_start + column * df_r;
            real_t x = FC + fr;
            real_t root_arg = x * x - term2;
            real_t safe_arg = root_arg > WKA_BULK_SAFE_SQRT_EPS ? root_arg : WKA_BULK_SAFE_SQRT_EPS;
...
#pragma HLS ALLOCATION operation instances = fsqrt limit = WKA_STOLT_OUT_LANES
#pragma HLS ALLOCATION operation instances = fdiv limit = WKA_STOLT_OUT_LANES
#pragma HLS ALLOCATION operation instances = fmul limit = WKA_STOLT_INTERP_FMUL_LIMIT
#pragma HLS ARRAY_PARTITION variable = WKA_STOLT_WEIGHT_ROM complete dim = 1
#pragma HLS BIND_STORAGE variable = WKA_STOLT_WEIGHT_ROM type = rom_2p impl = bram
#pragma HLS ARRAY_PARTITION variable = WKA_RANGE_WINDOW_ROM cyclic factor = WKA_STOLT_OUT_LANES dim = 1
#pragma HLS BIND_STORAGE variable = WKA_RANGE_WINDOW_ROM type = rom_2p impl = bram
...
INTERPOLATE:
    for (int column_base = 0; column_base < N; column_base += WKA_STOLT_OUT_LANES) {
...
#pragma HLS ARRAY_PARTITION variable = source cyclic factor = WKA_STOLT_ROW_PART_FACTOR dim = 1
#pragma HLS BIND_STORAGE variable = source type = ram_t2p impl = WKA_STOLT_ROW_STORAGE_IMPL
