...
static void load_tile_corner_turn(bus_t *__restrict in,
                                  bus_t tile[TILE_SIZE][TILE_SIZE / BUS_LANES], int ti, int tj) {
...
LOAD_TILE:
    for (int r = 0; r < TILE_SIZE; r++) {
        size_t offset = (static_cast<size_t>(row_base + r) * N + col_base) / BUS_LANES;
...
#pragma HLS UNROLL
                    ...
                    data_t value = unpack_data(static_cast<uint64_t>(source.range(
                        (source_lane + 1) * WKA_COMPLEX_SAMPLE_BITS - 1,
                        source_lane * WKA_COMPLEX_SAMPLE_BITS)));
                    float magnitude =
                        std::sqrt(value.real() * value.real() + value.imag() * value.imag());
...
STORE_COMPLEX_TILE:
    for (int c = 0; c < TILE_SIZE; c++) {
        size_t offset = (static_cast<size_t>(row_base + c) * N + col_base) / BUS_LANES;
        int source_word = c / BUS_LANES;
        int source_lane = c % BUS_LANES;
        for (int output_word = 0; output_word < TILE_SIZE / BUS_LANES; output_word++) {
#pragma HLS PIPELINE II = WKA_CORNER_LOAD_STORE_II
            bus_t packed = 0;
...
static void process_tile_pair_ping_to_pong(bus_t *__restrict in, bus_t *__restrict out,
                                           bus_t tile_ping[TILE_SIZE][TILE_SIZE / BUS_LANES],
                                           bus_t tile_pong[TILE_SIZE][TILE_SIZE / BUS_LANES],
                                           int prev_ti, int prev_tj, int next_ti, int next_tj,
                                           bool magnitude_output) {
#pragma HLS INLINE off
#pragma HLS DATAFLOW
...
static void process_tile_pair_pong_to_ping(bus_t *__restrict in, bus_t *__restrict out,
                                           bus_t tile_ping[TILE_SIZE][TILE_SIZE / BUS_LANES],
                                           bus_t tile_pong[TILE_SIZE][TILE_SIZE / BUS_LANES],
                                           int prev_ti, int prev_tj, int next_ti, int next_tj,
                                           bool magnitude_output) {
...
#pragma HLS INLINE off
    ...
    bus_t tile_ping[TILE_SIZE][TILE_SIZE / BUS_LANES];
    bus_t tile_pong[TILE_SIZE][TILE_SIZE / BUS_LANES];
#pragma HLS ARRAY_PARTITION variable = tile_ping cyclic factor = BUS_LANES dim = 1
#pragma HLS ARRAY_PARTITION variable = tile_pong cyclic factor = BUS_LANES dim = 1
...
#pragma HLS LOOP_FLATTEN off
        ...
        if (tile_idx & 1) {
            process_tile_pair_ping_to_pong(in, out, tile_ping, tile_pong, prev_ti, prev_tj, next_ti,
                                           next_tj, magnitude_output);
        } else {
            process_tile_pair_pong_to_ping(in, out, tile_ping, tile_pong, prev_ti, prev_tj, next_ti,
                                           next_tj, magnitude_output);
        }
    }
