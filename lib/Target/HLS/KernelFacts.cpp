...
static LogicalResult emitKernelFacts(Operation *root, llvm::raw_ostream &os) {
  ...
  uint64_t expensiveCount = 0;
  uint64_t callCount = 0;
  uint64_t maxFanout = 0;
  llvm::SetVector<unsigned> widths;
  SmallVector<std::pair<int64_t, unsigned>> transforms;
  SmallVector<bool> stridedTransforms;
  SmallVector<std::pair<uint64_t, uint64_t>> buffers;

  module.walk([&](Operation *op) {
    ++operationCount;
    StringRef name = op->getName().getStringRef();
    loadCount += name == "affine.load" || name == "memref.load";
    storeCount += name == "affine.store" || name == "memref.store";
    callCount += name == "func.call";
    expensiveCount += name == "arith.divf" || name == "math.sqrt" ||
                      name == "math.sin" || name == "math.cos" ||
                      name == "math.exp" || name == "math.log";
    for (Value result : op->getResults())
      maxFanout = std::max<uint64_t>(
          maxFanout, std::distance(result.use_begin(), result.use_end()));
    ...
    auto recordTransform = [&](ShapedType type, int64_t dim) {
      ...
      stridedTransforms.push_back(type.getRank() > 1 &&
                                  dim + 1 != type.getRank());
    };
  ...
  os << "],\"transform_strided\":[";
  for (auto [index, strided] : llvm::enumerate(stridedTransforms)) {
    if (index)
      os << ',';
    os << (strided ? "true" : "false");
  }
  os << "],\"buffers\":[";
