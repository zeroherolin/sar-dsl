...
struct CreateMemrefSubview
    : public sar::impl::CreateMemrefSubviewBase<CreateMemrefSubview> {
...
static void createSubviewBeforeLoopBand(AffineLoopBand band,
  ...
  band.back().walk([&](Operation *op) {
    ...
    // The inferred result type drops the memory space, which later leaves
    // the view and its base disagreeing about where they live. Carry it
    // across explicitly.
...
std::unique_ptr<Pass>
sar::createCreateMemrefSubviewPass(CreateSubviewMode createSubviewMode) {
