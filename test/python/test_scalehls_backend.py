...
from conftest import REPO_ROOT, requires_scalehls
...
def test_linalg_flow_emits_hls():
    """The opt-in HIDA-PyTorch flow handles elementwise and reduction
    kernels end to end."""

    @sar.func
    def scale_sum(a: sar.f32[N, N], b: sar.f32[N, N]) -> sar.f32[N]:
        return sar.sum(a * b, dim=1)

    design = scale_sum.compile(backend="scalehls", options={"flow": "linalg"})
    assert design.flow == "linalg"
...
def test_every_construct_emits_hls():
    ...
    for fn, types in cases:
        design = fn.specialize(*types).compile(backend="scalehls")
        assert design.flow == "affine", fn
...
def test_testbench_csim_passes(tmp_path):
    ...
    design = phase.compile(backend="scalehls")
...
def test_stages_become_a_dataflow_region():
    ...
    source = spectrum.compile(backend="scalehls").source()
...
def test_streamed_planes_are_shared_across_lifetimes():
    ...
    def ports(fn, name):
        fn.name = name
        source = fn.compile(backend="scalehls",
                            options={
                                "axi_interface": True,
                                "on_chip_budget": 1
                            }).source()
...
def test_broadcast_does_not_materialize_a_plane():
    ...
    source = phased.compile(backend="scalehls",
                            options={
                                "axi_interface": True,
                                "on_chip_budget": 1
                            }).source()
...
def test_streaming_passes_keep_full_rows():
    ...
    source = scale.compile(backend="scalehls",
                           options={
                               "axi_interface": True,
                               "on_chip_budget": 1
                           }).source()
...
def test_scratch_lands_in_block_ram():
    ...
    source = spectrum.compile(backend="scalehls",
                              options={
                                  "axi_interface": True
                              }).source()
...
def test_on_chip_budget_decides_placement():
    ...
    def emit(n, **options):
        ...
        return spectrum.compile(backend="scalehls",
                                options={
                                    "axi_interface": True,
                                    **options
                                })
...
def test_full_alos_raster_emits():
    ...
    design = build_kernel(16384,
                          ALOS_PARAMS).compile(backend="scalehls",
                                               options={"axi_interface": True})
    source = design.source()
