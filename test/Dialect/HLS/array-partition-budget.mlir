...
// RUN: sar-opt %s --hls-array-partition="lutram-max-bits=0 lutram-bytes=0 bram-bytes=16384 uram-bytes=0 bram-block-bytes=1024 uram-block-bytes=0" | FileCheck %s --check-prefix=GEOMETRY
...
func.func @banked_budget() attributes {top_func} {
  ...
  // GEOMETRY-LABEL: func.func @banked_budget
  // GEOMETRY: #hls.partition
