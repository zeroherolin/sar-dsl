// RUN: sar-opt -hls-balance-dataflow-node %s | FileCheck %s

// External buffers keep depth 1: a DRAM buffer is persistent, so a
// consumer several levels down simply reads it later, and where the
// levels do have to be held apart CreateTokenStream carries that with a
// token of the matching depth. Giving the buffer a depth here would
// duplicate that, at one full-size plane per level.
...
// CHECK:         } {parallel}
// CHECK:       }
// CHECK:       %0 = hls.dataflow.buffer {depth = 1 : i32} : memref<64x56x56xi8, #hls.mem<dram>>
...
// CHECK:       hls.dataflow.node(%0, %arg6, %3) -> (%4, %5) {inputTaps = [0 : i32, 0 : i32, 0 : i32], level = 2 : i32} : (memref<64x56x56xi8, #hls.mem<dram>>, memref<64x64xi8, #hls.mem<dram>>, memref<64x28x28xi8, #hls.mem<dram>>) -> (memref<64x28x28xi8, #hls.mem<dram>>, memref<64x28x28xi8, #hls.mem<dram>>) {
